...
<COPY> {
  ...
  <<EOF>>                       { throw ScannerException(file,ErrorMessages.EOF_IN_MACROS); }
}
...
<MACROS> {
  ...
  "%cupsym"{WSP}+{NNL}*       { throw ScannerException(file,ErrorMessages.QUIL_CUPSYM, yyline); }
  ...
  "%unicode"({WSP}+{DottedVersion})? { val v: String = yytext().substring(8).trim();
  ...
  "%buffer" {WSP}+ {NNL}*     { throw ScannerException(file,ErrorMessages.NO_BUFFER_SIZE, yyline); }
  "%initthrow" {WSP}+ {QUIL} {WSP}* { initThrow = concExc(initThrow,yytext().substring(11).trim()); }
  "%initthrow" {WSP}+ {NNL}*  { throw ScannerException(file,ErrorMessages.QUIL_INITTHROW, yyline); }
  "%eofthrow"  {WSP}+ {QUIL} {WSP}*  { eofThrow = concExc(eofThrow,yytext().substring(10).trim()); }
  "%eofthrow"  {WSP}+ {NNL}*  { throw ScannerException(file,ErrorMessages.QUIL_EOFTHROW, yyline); }
  ...
  "%yylexthrow"{WSP}+ {NNL}*  { throw ScannerException(file,ErrorMessages.QUIL_YYLEXTHROW, yyline); }
  "%throws"    {WSP}+ {NNL}*  { throw ScannerException(file,ErrorMessages.QUIL_THROW, yyline); }
  "%scanerror" {WSP}+ {QualIdent} {WSP}* { scanErrorException = yytext().substring(11).trim(); }
  "%scanerror" {WSP}+ {NNL}*  { throw ScannerException(file,ErrorMessages.QUIL_SCANERROR, yyline); }
  ...
  "%suppress" {WSP}+ {NNL}* { throw ScannerException(file, ErrorMessages.NOT_A_WARNING_ID, yyline); }
  ...
  "%no-warn" {WSP}+ {NNL}* { throw ScannerException(file, ErrorMessages.NOT_A_WARNING_ID, yyline); }
  ...
  "%warn" {WSP}+ {NNL}* { throw ScannerException(file, ErrorMessages.NOT_A_WARNING_ID, yyline); }
  ...
  "%"{Ident}                  { throw ScannerException(file,ErrorMessages.UNKNOWN_OPTION, yyline, yycolumn); }
...
<STATES> {
  ...
  <<EOF>>                     { throw ScannerException(file,ErrorMessages.EOF_IN_STATES); }
}
...
<REGEXP> {
  ...
  "<<EOF>>"               { throw ScannerException(file,ErrorMessages.EOF_WO_ACTION); }
  ...
  <CHARCLASS> {
    ...
    {WSPNL}*"\\p{"[^}]*"}"     { val trimmedText: String = yytext().trim();
...
<REPEATEXP> {
  ...
  <<EOF>>                 { throw ScannerException(file,ErrorMessages.EOF_IN_REGEXP); }
}
...
<STRING_CONTENT> {
  ...
  {NL}     { throw ScannerException(file,ErrorMessages.UNTERMINATED_STR, yyline, yycolumn); }

  {HexNumber} { string.append(Integer.parseInt(yytext().substring(2,yylength()), 16)); }
  {OctNumber} { string.append(Integer.parseInt(yytext().substring(1,yylength()), 8)); }
  ...
  <<EOF>>     { throw ScannerException(file,ErrorMessages.EOF_IN_STRING); }
}
...
<REGEXP, CHARCLASS> {
  ...
  \\b { return symbol(sym.CHAR, '\b'); }
  \\n { return symbol(sym.CHAR, '\n'); }
  \\t { return symbol(sym.CHAR, '\t'); }
  \\f { return symbol(sym.CHAR, '\u000c'); }
  \\r { return symbol(sym.CHAR, '\r'); }
...
<JAVA_CODE> {
  ...
  <<EOF>>     { throw ScannerException(file,ErrorMessages.EOF_IN_ACTION, action_line-1); }
}
...
<COMMENT> {
  ...
  <<EOF>>     { throw ScannerException(file,ErrorMessages.EOF_IN_COMMENT); }
}

<REGEXP_CODEPOINT_SEQUENCE> {
  "}"             { yybegin(REGEXP); return symbol(sym.STRING, string.toString()); }
  {HexDigit}{1,6} { val codePoint: Int = Integer.parseInt(yytext(), 16);
                    if (codePoint <= getMaximumCodePoint()) {
                      string.append(Character.toChars(codePoint));
                    } else {
...
<CHARCLASS_CODEPOINT> { // Specialized form: only one codepoint allowed, no whitespace allowed
  {HexDigit}{1,6} "}" { val codePoint: Int = Integer.parseInt(yytext().substring(0, yylength() - 1), 16);
                        if (codePoint <= getMaximumCodePoint()) {
                          yybegin(CHARCLASS);
                          return symbol(sym.CHAR, codePoint);
                        } else {
...
\R { throw ScannerException(file,ErrorMessages.UNEXPECTED_NL, yyline, yycolumn); }
